function output_value = run_comsol(alpha_val, th_W_ratio_val, fidelity_level_input, target_strain_percentage)
...
try
    ...
    if fidelity_level_input == 0.0 % Coarser Mesh for LF
        model.param.set('numX', 'L/2[mm]');
        model.param.set('numY', 'W/4[mm]');
    else % Finer Mesh for HF
        model.param.set('numX', 'L/1[mm]');
        model.param.set('numY', 'W/2[mm]');
    end

    %model.param.set('numX', 'L/numX_divisor_mm');
    %model.param.set('numY', 'W/numY_divisor_mm');
